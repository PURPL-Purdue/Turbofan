...
TF.turbine.HP.IN = REF_structs.Turbine_IN(
    ...
    40000,#TF.compressor.HP.OUT.RPM,       # rpm               RPM                                                 | RPM
    ...
    TF.cycle.OUT.Cps.cHP,           # Cp_c              Compressor specific heat at constant pressure       | J/kg*K
    TF.cycle.OUT.Cps.tHP,           # Cp_t              Turbine specific heat at constant pressure          | J/kg*K
    TF.cycle.IN.gamma.cHP,          # gamma_c           Comrpessor specific heat ratio                      | nondimensional
    TF.cycle.IN.gamma.tHP,          # gamma_t           Turbine specific heat ratio                         | nondimensional

    0.995,                          # eta_mech          Mechanical efficiencicy                             | nondimensional
    0.0,                            # ep                Cooling mass flow rate fraction                     | in decimal form

    # Fan Stuff
    None,                           # m_dot_f           Fan (total) mass flow rate                          | J/kg*K
    None,                           # Cp_f              Fan specific heat at constant pressure              | J/kg*K
    None,                           # T0_15             Fan inlet total temperature                         | K
...
TF.turbine.LP.IN = REF_structs.Turbine_IN(
    ...
    TF.compressor.LP.OUT.r_mean_1,  # r_mean_i          Inlet pitchline radius                              | m TODO make this HP outlet radius
